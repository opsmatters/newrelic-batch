...
/**
 * Renderer that converts email alert channels to a text file.
 * 
 * @author Gerald Curley (opsmatters)
 */
public class EmailChannelRenderer extends OutputFileRenderer<EmailChannel>
{
    private static final Logger logger = Logger.getLogger(EmailChannelRenderer.class.getName());

    /**
     * Private constructor.
     */
    private EmailChannelRenderer()
    {
    }
    ...
    {
        TemplateFactory.registerTemplate(EmailChannelRenderer.class, template);
    }
     ...
     */
    public static void write(List<EmailChannel> channels, OutputFileWriter writer) throws IOException
    {
        new EmailChannelRenderer().render(channels, writer);
    }
     ...
     */
    protected String[] serialize(FileTemplate template, EmailChannel channel)
    {
        List<String> line = new ArrayList<String>();
        line.add(channel.getName());
        line.add(template.getType());
        line.add(channel.getConfiguration().getRecipients());
        line.add(channel.getConfiguration().getIncludeJsonAttachment());
