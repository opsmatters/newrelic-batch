...
/**
 * Renderer that converts HipChat alert channels to a text file.
 * 
...
public class HipChatChannelRenderer extends OutputFileRenderer<HipChatChannel>
{
    ...
    {
        TemplateFactory.registerTemplate(HipChatChannelRenderer.class, template);
    }
     ...
     */
    public static void write(List<HipChatChannel> channels, OutputFileWriter writer) throws IOException
    {
        new HipChatChannelRenderer().render(channels, writer);
    }
     ...
     */
    protected String[] serialize(FileTemplate template, HipChatChannel channel)
    {
        List<String> line = new ArrayList<String>();
        line.add(channel.getName());
        line.add(template.getType());
        line.add(channel.getConfiguration().getAuthToken());
        line.add(channel.getConfiguration().getRoomId());
